...
class StepFunctionsStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        submit_transaction = sf_tasks.LambdaInvoke(self, "SubmitTransaction",
                                                   lambda_function=transaction_functions.submit_function,
                                                   payload=sfn.TaskInput.from_json_path_at("$"),
                                                   output_path="$.Payload")

        order_transaction = sf_tasks.LambdaInvoke(self, "OrderTransaction",
                                                  lambda_function=transaction_functions.order_function,
                                                  payload=sfn.TaskInput.from_json_path_at("$"),
                                                  output_path="$.Payload")

        cancel_transaction = sf_tasks.LambdaInvoke(self, "CancelTransaction",
                                                   lambda_function=transaction_functions.cancel_function,
                                                   payload=sfn.TaskInput.from_json_path_at("$"),
                                                   output_path="$.Payload")
        ...
        order_transaction.next(order_status)

        cancel_status = self.create_cancel_status_choices(succeed, transaction_fail)
        cancel_transaction.next(cancel_status)

        transaction_status = self.create_transaction_status_choices(cancel_transaction, order_transaction, submit_fail)
        definition = submit_transaction.next(transaction_status)
        ...
        CfnOutput(self, "HistoryTableName",
