...
const App = () => {
  ...
  useEffect(() => {
    ...
    const remainingForGrossSalary = CalculationUtils.calculateRemainingForGrossSalary(formData);
  ...
  }, [formData.hourly_rate, formData.hours_worked, formData.save_to_buffer, formData.pension_saving]);
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-white">
      ...
      <section className="md:w-[42%] bg-white p-8 md:p-16 flex flex-col justify-between relative overflow-hidden">
        ...
        <header className="relative z-10 flex items-center space-x-3 mb-16">
          ...
          <span className="font-extrabold text-2xl tracking-tighter text-slate-800">EightyTwenty.</span>
        </header>
        ...
        <div className="relative z-10 max-w-md">
          ...
          <p className="text-xl text-slate-500 mb-12 leading-relaxed font-medium">
            The all-in-one terminal for consultants. Automate your tax buffers, calculate your real take-home pay with the 80/20 model.
          </p>
        ...
        <footer className="relative z-10 mt-10 pt-2 border-t border-slate-100 hidden md:block">
          <div className="flex space-x-6 grayscale opacity-40">
            <span className="font-medium text-sm text-slate-700">By EightyTwenty AB.</span>
          </div>
        </footer>
      </section>
      ...
      <section className="md:w-[58%] bg-white p-8 md:p-16 flex flex-col">
        <div className="w-full max-w-2xl">
          ...
          <div className="flex items-center justify-between mb-10 pb-6 border-b border-slate-50">
            <div>
              <h2 className="text-2xl font-black text-slate-900">Guest Mode • Figures in SEK</h2>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
              ...
              <div className="space-y-1">
                <label className="input-label flex items-center"><Coins size={12} className="mr-1" /> Hourly Rate (SEK)</label>
                <div className="relative">
                  <input type="number" name="hourly_rate" value={formData.hourly_rate} onChange={handleChange} className="input-field font-mono" />
                </div>
              </div>
              <div className="space-y-1">
                <label className="input-label flex items-center"><Clock size={12} className="mr-1" /> Monthly Hours</label>
                <input type="number" name="hours_worked" value={formData.hours_worked} onChange={handleChange} className="input-field font-mono" />
              </div>

              <div className="space-y-1">
                <label className="input-label flex items-center"><PiggyBank size={12} className="mr-1" /> Buffer Savings</label>
                <div className="relative">
                  <input type="number" name="save_to_buffer" value={formData.save_to_buffer} onChange={handleChange} className="input-field font-mono text-brand-600" />
                </div>
              </div>

              <div className="space-y-1">
                <label className="input-label flex items-center"><TrendingUp size={12} className="mr-1" /> Pension Saving</label>
                <div className="relative">
                  <input type="number" name="pension_saving" value={formData.pension_saving} onChange={handleChange} className="input-field font-mono text-slate-500" />
                </div>
              </div>
              ...
              <div className="col-span-full grid grid-cols-3 gap-4 pt-4">
                <div className="p-5 rounded-2xl bg-brand-50 text-brand-700 border border-brand-100">
                  <span className="text-[10px] font-black uppercase text-brand-400 block mb-1">Invoiced Amount</span>
                ...
                <div className="p-5 rounded-2xl bg-slate-100 text-slate-900 border border-slate-200">
                  <span className="text-[10px] font-black uppercase text-slate-500 block mb-1">After Deduction (80%)</span>
                  <div className="text-xl font-bold font-mono text-slate-400 leading-none">
                    {formatCurrency(formData.after_deduction)}
                  </div>
                </div>
                <div className="p-5 rounded-2xl bg-brand-50 text-brand-700 border border-brand-100">
                  <span className="text-[10px] font-black uppercase text-brand-400 block mb-1">Gross Salary Basis</span>
                  <div className="text-xl font-bold font-mono leading-none">
              ...
              <div className="p-5 rounded-2xl bg-emerald-50 text-emerald-700 border border-emerald-100">
                <span className="text-[10px] font-black uppercase text-emerald-500 block mb-1">Remaining Salary after fixed costs</span>
                <div className="text-xl font-bold font-mono leading-none">
              ...
              <div className="p-5 rounded-2xl bg-slate-100 text-slate-900 border border-slate-200">
                <span className="text-[10px] font-black uppercase text-slate-500 block mb-1">Employer Fee</span>
                <div className="text-xl font-bold font-mono text-slate-400 leading-none">
              ...
              <div className="col-span-full grid grid-cols-1 gap-4 pt-4">
                <div className="p-5 rounded-2xl text-center bg-emerald-50 text-emerald-700 border border-emerald-100 w-full">
                  <span className="text-[10px] font-black uppercase text-emerald-500 block mb-1">Remaining Salary for Gross Salary</span>
                  <div className="text-xl font-bold font-mono leading-none">
            ...
            {/*<button type="submit" className="btn-primary space-x-3 text-lg py-5 shadow-[0_20px_40px_-10px_rgba(15,23,42,0.3)]">
              <Save size={20} />
